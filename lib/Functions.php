<?php 
    ...
    class Functions{
        ...
        public static function image_validation($data, $p_location, $size, $min_size, $max_size, $tmp_name){
            $target_dir = "all_img/";
            ...
            if ($ok == "ok") {
                if ($size > $max_size) {
                    $err_msg = "Maximum Image Size 2 MB";
                    Session::set_value($p_location, $err_msg);
                    $ok = "not ok";
                    self::$ok_alert = "not ok";
                }
            ...
            if ($ok == "ok") {
                if ($min_size > $size) {
                    $err_msg = "Minimum Image Size 20 KB";
                    Session::set_value($p_location, $err_msg);
                    $ok = "not ok";
                    self::$ok_alert = "not ok";
                }
