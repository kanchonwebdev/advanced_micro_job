        ...
        public static function data_validation($data, $name, $type, $min_len, $max_len, $val){
            if ($type == "text") {
                if (empty($data)) {
                    ...
                    Session::set_value("msg", $empty_err_msg);
            ...
            if ($type == "email") {
                if (empty($data)) {
                    $empty_err_msg = "<b>Error!</b> Email Field should not be Empty";
                    Session::set_value("msg", $empty_err_msg);
                    self::$ok_alert = "not ok";
                }
            }
            ...
            if ($type == "pass") {
                if (empty($data)) {
                    $empty_err_msg = "<b>Error!</b> Pass Field should not be Empty";
                    Session::set_value("msg", $empty_err_msg);
                    self::$ok_alert = "not ok";
                    return;
                }
            ...
            if ($type == "pass") {
                if (strlen($data) > $max_len) {
                    $empty_err_msg = "<b>Error!</b> Password Maximum Length ".$max_len;
                    Session::set_value("msg", $empty_err_msg);
                    self::$ok_alert = "not ok";
                    return;
                }
            ...
            if ($type == "pass") {
                if (strlen($data) < $min_len) {
                    $empty_err_msg = "<b>Error!</b> Password Minimum Length ".$min_len;
                    Session::set_value("msg", $empty_err_msg);
                    self::$ok_alert = "not ok";
                    return;
                }
        ...
        public static function other_validation($data, $name, $type){
            ...
            if ($type == "pass") {
                if (!preg_match("/^[a-zA-Z0-9]*$/",$data)) {
                    $empty_err_msg = "<b>Error!</b>Password Only Number and Letters Allowed";
                    Session::set_value("msg", $empty_err_msg);
                    self::$ok_alert = "not ok";
                    return;
                }
            ...
            if ($type == "pass") {
                if (preg_match("/^[0-9]*$/",$data)) {
                    $empty_err_msg = "<b>Error!</b>Password Only Number Not Allowed Please combine Number and Letter these";
                    Session::set_value("msg", $empty_err_msg);
                    self::$ok_alert = "not ok";
                    return;
                }
            ...
            if ($type == "pass") {
                if ((preg_match("/^[a-zA-Z]*$/",$data))) {
                    $empty_err_msg = "<b>Error!</b>Password Only Letters Not Allowed Please combine Number and Letters";
                    Session::set_value("msg", $empty_err_msg);
                    self::$ok_alert = "not ok";
                    return;
                }
