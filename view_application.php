...
<body>
    ...
    <?php
        if ($data != false) {
            ...
            $j_data = $s_que->check_job_apply_2($a_id)->fetch_assoc();
            $e_data = $s_que->select_job_seeker_by_u_name($j_id_2['apply_by'])->fetch_assoc();
    ?>
            ...
            <div class="single-job-container">
                <div class="single-job-left">
                    ...
                    <div class="single-job-left-footer">
                        <a href="job_seeker_profile.php?u_id=<?php echo $e_data['u_id'] ?>" class="btn" target="_blank">View Profile</a>
                    </div>
                </div>
                ...
                <div class="single-job-right">
                        ...
                        <div class="single-job-right-header-content">
                            ...
                            <div class="single-job-right-header-block">
                                ...
                                <p>
                                    ...
                                    ?> 
                                    by <b><?php echo $data2['u_name'] ?></b></p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="single-job-right-footer">
                        ...
                        <?php
                            $a_data = $s_que->check_job_apply_2($a_id);
                            if ($a_data !=  false) {
                                $a_data_2 = $a_data->fetch_assoc();
                        ?>

                            <form method="POST" action="reject_code.php" class="single-job-right-footer-header">
                                <input type="hidden" name="j_id" value="<?php echo $data2['j_id'] ?>">
                                <input type="hidden" name="j_apply_by" value="<?php echo $j_id_2['apply_by'] ?>">

                                <div>
                                    <b>Apply Interest: </b><br>
                                    <?php echo $a_data_2['a_description'] ?>
                                </div>
                                
                                <div style="display: flex;align-itmes: center; justify-content: center">
                                    <?php if ($a_data_2['a_status'] == "Submitted") {?>
                                        <a href="send_offer.php" class="btn">Send Offer</a>
                                        <input type="submit" value="Reject" class="btn" name="reject">
                                    <?php }elseif($a_data_2['a_status'] == "Offer Reject" || $a_data_2['a_status'] == "Reject"){ ?>
                                        <p>Job Offer Reject</p>
                                    <?php }elseif($a_data_2['a_status'] == "Hired"){ ?>
                                        
                                        <?php 
                                            $check_rating = $s_que->check_rating($a_data_2['apply_by'], $a_data_2['post_by'], $a_data_2['job_id'], $a_id);
                                            if ($check_rating != false) {
                                                echo "You rate already";
                                            }else{    
                                        ?>
    ...
    <?php
        $_SESSION['apply_info'] = array($j_data['job_id'], $j_data['apply_id'], $j_data['post_by'], $j_data['apply_by']);
    ?>
